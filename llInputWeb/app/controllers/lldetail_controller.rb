class LldetailController < ApplicationController
	@@mutex_main_data = Mutex.new 
	@@mutex_file = Mutex.new
	@@main_data = []
	$guishudi_hash = {"1"=> "本地" ,"2"=>"外地","3"=>"本地外地"}
	@@hash_array = [nil,nil,nil,$guishudi_hash]
	DataArrayLength = 4  
	...
	def edit
		id = params[:id]    
		item = create_or_search(id:id, main_data:@@main_data, data_mutex:@@mutex_main_data){|new_item,maxId|new_item << maxId << "110000" << ""<< "1"}
		if item
			@item = item
		else
			render html: "<strong>未找到该条限行描述，是不是另一个人正在删除它？</strong>".html_safe , layout: "application"
		end
	end
...
def edit_item
	...
	if del
		...
		@@mutex_main_data.synchronize{
			item.clear
			item <<xuhao<< city_code <<detail<<guishudi     
		}
	...
	if type
		...
		if type=="lldetail"
			...
			if data && data.length>0
				file_name =  File.join(dirName,"outputLLDetailData.xls")
				File.delete(file_name) if File.exist?(file_name)	
				@@mutex_file.synchronize{  
					ensureDir(dirName)
					if write_xlsx(file_name,data)   
						sendFile(file_name,"限行描述导出.xls")		
					else
						file_name =  File.join(dirName,"outputLLDetailData.txt")
						File.delete(file_name) if File.exist?(file_name)	
						if Write(file_name,data)
							sendFile(file_name,"限行描述导出.txt")	
						end
					end
				}
				return
